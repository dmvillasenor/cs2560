...
public:
    PreferredCustomer(double amount, int cNum, bool mList, std::string ln, std::string fn,
                      std::string add, std::string c, std::string s, int z, int p ) :
                      CustomerData( cNum, mList, ln, fn, add, c, s, z, p), purchaseAmount(amount){
...
int main() {
    ...
    PreferredCustomer customer(amount, 123, true, "Obama", "Barack", "12345 Box Street", "Boxville", "Boxifornia", 12345, 1234567890);
    ...
    std::cout << "First: " << customer.getFirstName() << std::endl
              ...
              << "Purchases: $" << customer.getPurchaseAmount() << std::endl
              << "Discount Percent: " << customer.getDiscount() << std::endl;
